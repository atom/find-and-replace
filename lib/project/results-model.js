...
module.exports =
(ResultsModel = class ResultsModel {
  ...
  search(findPattern, pathsPattern, replacePattern, options) {
    ...
    this.inProgressSearchPromise = atom.workspace.scan(this.regex, {paths: searchPaths,
      onPathsSearched, leadingContextLineCount, trailingContextLineCount}, (result, error) => {
      if (result) {
  ...
  setActive(isActive) {
    if ((isActive && this.findOptions.findPattern) || !isActive) { return this.active = isActive; }
  }
