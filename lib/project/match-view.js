...
class MatchView {
  ...
  render() {
    ...
    const {linesBefore, linesAfter} = this.match;
    ...
    return (
      $.li(
        ...
        $.ul({className: 'list-tree'},
          linesBefore ? linesBefore.map((line, index) =>
            $.li({className: 'list-item'},
              $.span({className: 'line-number text-subtle'}, range.start.row + 1 - linesBefore.length + index),
          ...
          linesAfter ? linesAfter.map((line, index) =>
            $.li({className: 'list-item'},
