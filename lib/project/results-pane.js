  ...
  render() {
    ...
    return (
      etch.dom.div(
        ...
        etch.dom.div({className: 'preview-header'},
          etch.dom.span({
            ref: 'previewCount',
            className: 'preview-count inline-block',
            innerHTML: this.isLoading ? 'Searching...' : getSearchResultsMessage(this.searchResults)
          }),
          ...
          etch.dom.div(
            ...
            etch.dom.div({className: 'btn-group'},
              etch.dom.button({ref: 'collapseAll', className: 'btn'}, 'Collapse All'),
              etch.dom.button({ref: 'expandAll', className: 'btn'}, 'Expand All')
            )
          ),

          etch.dom.div({className: 'inline-block', style: showIf(this.isLoading)},
            etch.dom.div({className: 'loading loading-spinner-tiny inline-block'}),
            ...
            etch.dom.div(
              ...
              etch.dom.span({ref: 'searchedCount', className: 'searched-count'},
                this.numberOfPathsSearched.toString()
              ),
              etch.dom.span({}, ' paths searched')
            )
          )
        ),
        ...
        etch.dom.ul(
          ...
          ...this.searchErrors.map(message =>
            etch.dom.li({className: 'text-error'}, escapeHtml(message))
          )
        ),
