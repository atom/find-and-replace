...
class ResultView {
  ...
  renderList() {
    ...
    for (; i < this.matches.length; i++) {
      ...
      if (match.linesBefore) itemBottomPosition += match.linesBefore.length * this.contextLineHeight;
      if (match.linesAfter) itemBottomPosition += match.linesAfter.length * this.contextLineHeight;
