...
class ResultsView {
      ...
      $.ol(
        ...
        ...this.searchResults.map(({filePath, matches}, i) => {
          ...
          return $(ResultView, {
            ...
            regex, replacePattern, previewStyle: this.previewStyle
          });
        })
      )
    );
  ...
  removeResult({filePath}) {
    const index = this.searchResults.findIndex(result => result.filePath === filePath);
    if (index !== -1) {
  ...
  moveDown() {
    ...
    this.renderResults().then(this.scrollToSelectedMatch.bind(this));
  }
  ...
  copyPath() {
    const {filePath} = this.searchResults[this.selectedResultIndex];
