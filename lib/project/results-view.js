...
class ResultsView {
  constructor({model}) {
    ...
    const visibilityChangedObserver = new IntersectionObserver(() => {
      this.invalidateItemHeights()
      visibilityChangedObserver.disconnect()
    });
    visibilityChangedObserver.observe(this.element);
  ...
  pageUp() {
    if (this.refs.listView) {
      ...
      this.setScrollTop(this.getScrollTop() - this.refs.listView.element.clientHeight);
      this.selectResultAtPosition(
        position - this.refs.listView.element.clientHeight,
  ...
  pageDown() {
    if (this.refs.listView) {
      const position = this.positionOfSelectedResult(this.pathDetailsHeight, this.matchHeight);
      const newScrollTop = this.getScrollTop() + this.refs.listView.element.clientHeight;
  ...
  selectResultAtPosition(position, pathDetailsHeight, matchHeight) {
    if (this.refs.listView) {
      ...
      if (this.getScrollTop() < bottom - this.refs.listView.clientHeight) {
        this.setScrollTop(bottom - this.refs.listView.clientHeight);
      }
