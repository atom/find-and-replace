...
class ResultsView {
      ...
      $.ol(
        ...
        $(ResultView, {
          ...
          item: {
            ...
            matches: [{
              range: [[0, 1], [0, 2]],
              linesBefore: ['test-line-before'],
              linesAfter: ['test-line-after'],
              lineTextOffset: 1,
  ...
  heightForSearchResult(searchResult, index) {
    ...
    if (!this.collapsedResultIndices[index]) {
      for (let i = 0, n = searchResult.matches.length; i < n; i++) {
        ...
        if (match.linesBefore) result += this.contextLineHeight * match.linesBefore.length;
        if (match.linesAfter) result += this.contextLineHeight * match.linesAfter.length;
      }
    }
    return result;
  }
  ...
  positionOfSelectedResult() {
    ...
    if (!this.collapsedResultIndices[this.selectedResultIndex] && this.selectedMatchIndex !== -1) {
      ...
      if (selectedMatch.linesBefore) result += this.contextLineHeight * selectedMatch.linesBefore.length;
    }
      ...
      for (let i = 0; i < this.model.getPathCount(); i++) {
        ...
        if (!this.collapsedResultIndices[i]) {
          ...
          for (let j = 0; j < searchResult.matches.length; j++) {
            ...
            if (match.linesBefore) bottom += this.contextLineHeight * match.linesBefore.length;
            if (match.linesAfter) bottom += this.contextLineHeight * match.linesAfter.length;
