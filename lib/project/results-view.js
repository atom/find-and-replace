...
class ResultsView {
  constructor({model}) {
    ...
    this.element.addEventListener('scroll', this.renderResults.bind(this));
    this.element.addEventListener('resize', this.renderResults.bind(this));
  ...
  renderResults({renderAll, renderNext} = {}) {
    for (const filePath of this.model.getPaths().slice(this.searchResults.length)) {
      if (!renderAll && !this.shouldRenderMoreResults()) break;
  ...
  shouldRenderMoreResults() {
    ...
    return renderedHeight < this.element.scrollTop + this.element.offsetHeight + this.pixelOverdraw;
  }
  ...
  moveToBottom() {
    return this.renderResults({renderAll: true}).then(() => {
  ...
  pageDown() {
    ...
    const position = this.positionOfSelectedResult(pathDetailsHeight, matchHeight);
    this.element.scrollTop += this.element.offsetHeight;
    return this.renderResults().then(() => {
      this.selectResultAtPosition(
        position + this.element.offsetHeight,
        pathDetailsHeight,
        matchHeight
  ...
  selectResultAtPosition(position, pathDetailsHeight, matchHeight) {
    let top = 0;
    for (let i = 0; i < this.searchResults.length; i++) {
      const detailsBottom = top + pathDetailsHeight;
      if (detailsBottom > position) {
      ...
      if (!this.collapsedResultIndices[i]) {
        const matchesBottom = detailsBottom + this.searchResults[i].matches.length * matchHeight;
        if (matchesBottom > position) {
          this.selectedResultIndex = i;
          this.selectedMatchIndex = Math.floor((position - detailsBottom) / matchHeight);
          break;
        }
  ...
  expandAllResults() {
    ...
    this.renderResults({renderAll: true});
  }
