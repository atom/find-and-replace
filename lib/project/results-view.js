...
class ResultsView {
  ...
  handleClick(event) {
    ...
    const clickedItemBox = event.target.closest('.list-item div');
    ...
    if (clickedItemBox && clickedItem.firstElementChild.matches('.context-row, .match-row')) {
  ...
  selectRow(i) {
    ...
    if (resultRow instanceof LeadingContextRow) {
      this.selectedRowIndex += resultRow.dy
    } else if (resultRow instanceof TrailingContextRow) {
      this.selectedRowIndex -= resultRow.dy
    }
